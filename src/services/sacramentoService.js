...
class SacramentoService {
  async getAllSacramentos(token) {
    try {
      ...
      const sacramentosEnriquecidos = await this.enrichSacramentos(response.data.sacramento, token);
  ...
  async enrichSacramentos(sacramentos, token) {
    try {
      const usuariosIds = [...new Set(sacramentos.map(s => s.usuario_id_usuario))];
      const parroquiasIds = [...new Set(sacramentos.map(s => s.institucion_parroquia_id_parroquia))];
      const tiposIds = [...new Set(sacramentos.map(s => s.tipo_sacramento_id_tipo))];
      const usuarios = await this.getUsuarios(usuariosIds, token);
      console.log(`Usuarios obtenidos: ${usuarios.length}`);
      const parroquias = await this.getParroquias(parroquiasIds, token);    
     // console.log(`Tipos de sacramentos que estan saliendo`);
      const tipos = await this.getTipos(tiposIds, token);
  ...
  async getTipos(ids, token) {
    try {
      ...
      // Buscar tipos en diferentes posibles estructuras
      let tipos = [];
  ...
  async getFilteredSacramentos(filter = {}, token = '') {
    try {
      ...
      if (filter.tipo_sacramento_id_tipo) {
        ...
        console.log(`Filtrado por tipo_sacramento_id: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.institucion_parroquia_id_parroquia) {
        ...
        console.log(`Filtrado por parroquia_id: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.usuario_id_usuario) {
        ...
        console.log(`Filtrado por usuario_id: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.activo !== undefined && filter.activo !== null) {
        ...
        console.log(`Filtrado por activo=${filter.activo}: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.foja) {
        ...
        console.log(`Filtrado por foja: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.numero) {
        ...
        console.log(`Filtrado por número exacto: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.numero_desde) {
        ...
        console.log(`Filtrado por numero_desde: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.numero_hasta) {
        ...
        console.log(`Filtrado por numero_hasta: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.anio_sacramento) {
        ...
        console.log(`Filtrado por año sacramento: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.mes_sacramento) {
        ...
        console.log(`Filtrado por mes sacramento: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.fecha_sacramento_desde) {
        ...
        console.log(`Filtrado por fecha_sacramento_desde: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.fecha_sacramento_hasta) {
        ...
        console.log(`Filtrado por fecha_sacramento_hasta: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.anio_registro) {
        ...
        console.log(`Filtrado por año registro: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.fecha_registro_desde) {
        ...
        console.log(`Filtrado por fecha_registro_desde: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.fecha_registro_hasta) {
        ...
        console.log(`Filtrado por fecha_registro_hasta: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.fecha_actualizacion_desde) {
        ...
        console.log(`Filtrado por fecha_actualizacion_desde: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.fecha_actualizacion_hasta) {
        ...
        console.log(`Filtrado por fecha_actualizacion_hasta: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.search) {
        ...
        console.log(`Filtrado por búsqueda general: ${sacramentos.length} resultados`);
      }
      ...
      if (filter.limit || filter.offset) {
        ...
        console.log(`Paginación aplicada: offset=${offset}, limit=${limit}`);
      }
  ...
  async getEstadisticas(filter = {}, token = '') {
    try {
      const sacramentos = await this.getFilteredSacramentos(filter, token);
