  ...
  Query: {
    ...
    sacramentosPorTipo: async (_, { tipo_id, filter = {} }, { token }) => {
      try {
        const filterConTipo = {
          ...filter,
          tipo_sacramento_id_tipo: tipo_id
        };
    ...
    sacramentosPorParroquia: async (_, { parroquia_id, filter = {} }, { token }) => {
      try {
        const filterConParroquia = {
          ...filter,
          institucion_parroquia_id_parroquia: parroquia_id
        };
    ...
    generarReportePDF: async (
      ...
      try {
        ...
        if (incluirEstadisticas) {
          estadisticas = await sacramentoService.getEstadisticas(
            filter,
            token,
            sacramentos
          );
